...
EmbeddedProvider.prototype.adaptOData = function (odataServer) {
  ...
  odataServer.model(this._model)
    .onNeDB(function (col, cb) {
      ...
      self.collections[col].beforeQuery(query, req).then(function () {
        cb()
      }).catch(cb)
    }).beforeUpdate(function (col, query, update, req, cb) {
      self.collections[col].beforeUpdate(query, update, req).then(function () {
        cb()
      }).catch(cb)
    }).beforeRemove(function (col, query, req, cb) {
      self.collections[col].beforeRemove(query, req).then(function () {
        cb()
      }).catch(cb)
    }).beforeInsert(function (col, doc, req, cb) {
      self.collections[col].beforeInsert(doc, req).then(function () {
        cb()
      }).catch(cb)
    })
}
