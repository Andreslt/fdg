...
function evaluateAssets (reporter, stringToReplace, req) {
  ...
  function convert (str, p1, offset, s, done) {
    ...
    if (p1.indexOf(' @') !== -1) {
      ...
      if (paramValue !== 'base64' && paramValue !== 'utf8') {
        throw new Error('Unsupported asset encoding param value ' + paramValue + ', supported values are base64 and utf8')
      }
    ...
    reporter.documentStore.collection('assets').find({ name: assetName }, req).then(function (result) {
      if (result.length < 1) {
        ...
        return readFile(reporter, assetName).then(function (content) {
          return new Buffer(content).toString(encoding)
        })
      }
      ...
      return new Buffer(result[0].content).toString(encoding)
