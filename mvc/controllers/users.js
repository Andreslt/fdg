...
router.post('/register', function (req, res) {
	...
	req.checkBody('company_id', 'Company is required').notEmpty();
	...
	if (errors) {
		req.flash('error', 'El valor del campo ' + errors[0].param + ' no es válido. Revíselo e intente nuevamente.')
		...
		User.createUser(newUser, function (err, user) {
			...
			if (err) {
				req.flash('error', 'Ha ocurrido un error. Al parecer la ID ya se encuentra registrada ' + err.errors+' e inténtelo nuevamente.');
				res.redirect('/users/register');
			}else {
...
router.post('/account/edit', (req, res) => {
	...
	form.parse(req, function (err, fields, files) {
		company.findOne({ _id: user.company_id }, (err, cny) => {
			...
			UserModule.findOneAndUpdate({ _id: user.id }, {
				$set: {
					...
					cellphone: fields.cellphone, image: fSImagePath_cloudinary
				}
			}, { new: true }, (err, usr) => {
